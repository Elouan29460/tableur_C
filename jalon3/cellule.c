    ...
    if (cellule->chaine[0] == '=') {
        ...
        while (token_str != NULL) {
            ...
            if (is_number(token_str)) {
                ...
                // Ajouter la cellule référencée dans les dépendances de la cellule courante
                s_cell *ref_cell = get_cell_by_reference(token_str);
                if (ref_cell != NULL) {
                    cellule->dependencies = list_append(cellule->dependencies, ref_cell);
                    // Ajouter la cellule courante dans les successeurs de la cellule référencée
                    ref_cell->successors = list_append(ref_cell->successors, cellule);
                }
